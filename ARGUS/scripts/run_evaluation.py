...
def main() -> None:
    ...
    config = EvaluationConfig(dataset_root=args.dataset, max_samples_per_category=args.max_samples)
    summary = EvaluationRunner(config).run()
    metadata = {"timestamp": datetime.now(timezone.utc).isoformat(), "dataset_root": str(config.dataset_root), "seed": config.seed, "attack_parameters": config.attack_parameters, "thresholds": {"attack": 0.70, "trust": 0.70, "verification": 0.70}, "git_commit": _git_commit()}
