...
async function postImage<T>(path: string, image: File, fields: Record<string, string> = {}): Promise<T> {
  const formData = new FormData()
  formData.append('image', image)
...
export function generateAttack(image: File, fields: Record<string, string>): Promise<AttackResult> {
  return postImage<AttackResult>('/attacks/generate', image, fields)
}

export function evaluateAttack(image: File, fields: Record<string, string>): Promise<AttackResult> {
