...
async def infer(image: UploadFile) -> InferenceResponse:
    ...
    prediction = get_model("resnet18").predict(decoded_image)
...
async def analyze(
    ...
    try:
        ...
        request_id = str(uuid4())
        result = ArgusPipeline().run(PipelineContext(request_id, decoded, {"model": get_model("resnet18"), "mode": mode, "attack_type": attack_type}))
